...
+ (void)savePreprocess:(id)preprocess andPostprocess:(id)postprocess withItem:(BIItem*)item forMethodName:(NSString*)methodName
{
  if (preprocess) {
    // Save preprocess
    SEL preprocessSel = sel_registerName([[BILibUtils preprocessNameForMethodName:methodName index:(int)[item numberOfPreprocess]] UTF8String]);
  ...
  if (postprocess) {
    // Save postprocess
    SEL postprocessSel = sel_registerName([[BILibUtils postprocessNameForMethodName:methodName index:(int)[item numberOfPostprocess]] UTF8String]);
    [BILibUtils addMethodToClass:item.targetClass
